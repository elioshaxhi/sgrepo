...
import org.seriousgame.repository.client.fieldsetsContext.*;
 ...
 * @author nomecognome
 */
public class ChieldrenAnalysisFieldSet extends FieldSet {
	...
	public ChieldrenAnalysisFieldSet(final MainEntryPointInterface mainEntryPointInterface, List<GenericModel> coda, List<GenericModelWithRelation> subCoda, List<GenericModelWithRelation> subSubCoda) {
...
//	}
	public void setTopicData(GenericModel model) {
		...
		if (model.get(ModelTag.relationID.toString()) != null) {
			theSubList = this.mainEntryPointInterface.getDatabase().getRelationSubList(this.subCoda, model.getId());
		...
		if (valType.equals("Soft skills")) {
			...
			theSubSubList = this.mainEntryPointInterface.getDatabase().getRelationSubList(this.subSubCoda, tmp);
