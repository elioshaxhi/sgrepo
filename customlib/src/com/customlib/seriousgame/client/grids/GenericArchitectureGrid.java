 ...
 * @author nertil
 */
public class GenericArchitectureGrid extends ContentPanel {
	...
	public GenericArchitectureGrid(final MainEntryPointInterface entrypoint, final String header, ArrayList<GenericModel> sourceData) {
		...
		b_add.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				GenericArchitectureForm form = new GenericArchitectureForm(entrypoint, Constants.Strings.b_new() + " " + header, components);
				form.addListener(Events.Hide, new Listener<WindowEvent>() {

					@Override
					public void handleEvent(WindowEvent be) {
						GenericArchitectureForm f = (GenericArchitectureForm) be.getSource();
						if (f.isOkClose()) {
							tmp = f.getArchitectureModel();
							addModel(tmp);
						}
					}
				});
		...
		Button b_edit = new Button(Constants.Strings.b_edit(), IconHelper.create("images/edit.png"), new SelectionListener<ButtonEvent>() {
			...
			@Override
			public void componentSelected(ButtonEvent ce) {
				GenericArchitectureForm form = new GenericArchitectureForm(entrypoint, Constants.Strings.b_edit() + " " + header, components);
				if (getSelection() == null) {
					MessageBox.alert(Constants.Strings.alert(), Constants.Strings.zeroSelection(), null);
				} else {
					form.setArchitectureModel(getSelection());
					form.addListener(Events.Hide, new Listener<WindowEvent>() {
